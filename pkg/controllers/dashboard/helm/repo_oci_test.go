...
func TestShouldResetRetries(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:              "Should reset retries if interval has not passed and forceUpdate is after the last status update",
			ociDownloadedTime: time.Date(2024, 04, 23, 10, 0, 0, 0, time.UTC),
			lastStatusUpdate:  &metav1.Time{Time: time.Date(2024, 04, 23, 10, 1, 0, 0, time.UTC)},
			forceUpdate:       &metav1.Time{Time: time.Date(2024, 04, 23, 10, 2, 0, 0, time.UTC)},
			timeNow: func() time.Time {
				return time.Date(2024, 04, 23, 10, 10, 0, 0, time.UTC)
			},
			expected: true,
		},
		{
			name:              "Should NOT reset retries if interval has not passed and forceUpdate is not nil but is before the last status update",
			ociDownloadedTime: time.Date(2024, 04, 23, 10, 0, 0, 0, time.UTC),
			lastStatusUpdate:  &metav1.Time{Time: time.Date(2024, 04, 23, 10, 2, 0, 0, time.UTC)},
			forceUpdate:       &metav1.Time{Time: time.Date(2024, 04, 23, 10, 1, 0, 0, time.UTC)},
			timeNow: func() time.Time {
				return time.Date(2024, 04, 23, 10, 10, 0, 0, time.UTC)
			},
			expected: false,
