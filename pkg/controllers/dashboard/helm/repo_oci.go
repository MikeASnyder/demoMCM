...
func getRetryPolicy(clusterRepo *catalog.ClusterRepo) retryPolicy {
	...
	if clusterRepo.Spec.ExponentialBackOffValues != nil {
		...
		if clusterRepo.Spec.ExponentialBackOffValues.MaxWait != nil {
			retryPolicy.MaxWait = clusterRepo.Spec.ExponentialBackOffValues.MaxWait.Duration
		}
		if clusterRepo.Spec.ExponentialBackOffValues.MinWait != nil {
			retryPolicy.MinWait = clusterRepo.Spec.ExponentialBackOffValues.MinWait.Duration
		}
	}
	return retryPolicy
}
...
func shouldResetRetries(clusterRepo *catalog.ClusterRepo) bool {
	...
	if clusterRepo.Spec.ForceUpdate != nil && !lastStatusUpdate.IsZero() && clusterRepo.Spec.ForceUpdate.After(lastStatusUpdate) {
		return true
	}
	return false
}
