...
// the supportconfig tarball
func (h *Handler) checkAuthorization(cspNamespace string, cspConfigmap string, writer http.ResponseWriter, request *http.Request) {
...
func (h *Handler) ServeHTTP(writer http.ResponseWriter, request *http.Request) {
	...
	if usePAYG := request.URL.Query()["usePAYG"]; len(usePAYG) > 0 && usePAYG[0] == "true" {
		...
		h.checkAuthorization(cspChartNamespace, cspMeteringArchiveConfigmap, writer, request)
	} else {
	...
	h.checkAuthorization(cspChartNamespace, cspAdapterConfigmap, writer, request)
