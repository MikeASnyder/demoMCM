...
func (pr *ProjectRolesTestSuite) testSetupUserAndProject() (*rancher.Client, *management.Project) {
	...
	createProjectAsAdmin, err := standardUserClient.Management.Project.Create(projects.NewProjectConfig(pr.cluster.ID))
	require.NoError(pr.T(), err)
...
func (pr *ProjectRolesTestSuite) TestManageProjectUserRoleCannotAddProjectOwner() {
	...
	addNewUserAsPO, addNewUserAsPOClient, err := rbac.SetupUser(pr.client, rbac.StandardUser.String())
	require.NoError(pr.T(), err)

	errUserRole2 := users.AddProjectMember(additionalUserClient, adminProject, addNewUserAsPO, rbac.ProjectOwner.String(), nil)
	require.Error(pr.T(), errUserRole2)
	assert.Contains(pr.T(), err.Error(), "422 Unprocessable Entity")
